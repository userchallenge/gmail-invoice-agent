...
def validate_credentials(config: dict):
    ...
    if not os.path.exists(gmail_creds):
        ...
        print("Please download credentials.json from Google Cloud Console and place it at this location")
        sys.exit(1)
...
def process_emails(gmail_server: GmailServer, classifier: EmailClassifier, config: dict) -> list:
    ...
    for email in tqdm(emails, desc="Processing emails"):
        try:
            ...
            logging.error(f"Error processing email {email.get('subject', 'Unknown')}: {e}")
            continue
...
def print_results(invoice_data: list, stats: dict):
    ...
    if stats:
        ...
        if 'top_vendors' in stats:
            print(f"   • Top vendors: {', '.join(list(stats['top_vendors'].keys())[:3])}")
...
Examples:
  ...
  python demo.py --days-back 7            # Process last 7 days only
    ...
    parser.add_argument('--days-back', 
                       type=int,
                       help='Number of days back to search (overrides config)')
    
    parser.add_argument('--max-emails',
                       type=int, 
    ...
    try:
        ...
        if invoice_data:
            ...
            if success:
                ...
                print(f"📄 Invoice data exported to: {config['output']['invoices_file']}")
                ...
                if len(invoice_data) > 0:
                    ...
                    print(f"   • Amount: {sample.get('amount', 'N/A')} {sample.get('currency', 'SEK')}")
